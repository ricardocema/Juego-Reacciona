//Aquí se importan las librerías necesarias para la ejecución del juego, en este caso, se usarán las librerías necesarias para usar SWING.
...
//Aquí se implementan los métodos usados en las clases General, GeneralAbiertos y Retos.
//Solo se comentará la primera pregunta de selección, la primera instrucción de la clase tutorial, la primera pregunta abierta y la primera pregunta de Reto, ya que las demás siguen la misma lógica
//PREGUNTA 1
//Aquí se crea la clase Nivel y se hace referencia a que se estará usando la clase General, ya que es la que contiene los métodos que se usarán
class Nivel extends General implements ActionListener{
	public Nivel() {
		super();
		//Se llama al método preguntaSeleccion1
		PreguntaSeleccion1();
		
	}
	//Se usa el evento ActionEvent en los botones de respuesta
	public void actionPerformed(ActionEvent e) {
		//Se indica lo que se realizará cuando se de click en el boton correcto
		if (e.getSource().equals(BotonB)) {
			//Se invoca el método Maspuntos, el cual aumenta el puntaje en 5
			puntaje=Maspuntos(puntaje);
			//Se invoca el método correcto, el cual aumenta la variable correctas en 1
			correctas=correcto(correctas);
			//Se despliega un mensaje indicando que la respuesta fue correcta, además indica el puntaje y en número de respuestas correctas hasta el momento
		...
		//Se indica lo que pasa cuando el se hace click en un botón incorrecto
		if(e.getSource().equals(BotonA)) {
			//Se invoca el método Menospuntos, el cual disminuye el puntaje en 3
...
//A continuación se crea la primera instrucción
...
//A continuación se crea la primera pregunta abierta
//PREGUNTA ABIERTA 01
//Se crea la clase Desafio01 y se indica que se usará la clase GeneralAbiertos
class Desafio01 extends GeneralAbiertos implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		//Si se hace click en el botón comprobar:
		if (e.getSource().equals(BotonComprobar)) {
			...
			if(Respuesta.getText().toLowerCase().contentEquals(Respuesta01.toLowerCase())) {
				intentos=intentos+2;
				//Se despliega un mensaje indicando que se respondió la respuesta de forma correcta, además indica los puntos de intento restantes y el puntaje
				JOptionPane.showMessageDialog(null, "Respuesta correcta. Aprobaste el primer desafío y te quedan " + intentos + " puntos de intentos restantes y " + General.puntaje + " puntos");
			...
			//Si la respuesta introducida no concuerda con el texto almacenado en la variable Respuesta01, entonces la variable intentos disminuye en 1
				...
				intentos=intentos-1;
				//Se despliega un mensaje indicando que se falló en la respuesta y además indica cuantos puntos de intento quedan.
				...
				if(intentos==0) {
					intentos=intentos+1;
					//Se despliega un mensaje indicando que se falló en el desafío y que debemos intercambiar puntos si queremos continuar jugando
					JOptionPane.showMessageDialog(null, "Respuesta incorrecta. No completaste el primer desafío y no tienes más puntos de intentos para avanzar. Intercambia puntos por de intento para seguir avanzando");
				}
			}
		}
		//Aquí se indica lo que pasa si se hace click en el botón conseguir
		if (e.getSource().equals(BotonConseguir)) {
			//Si el puntaje es menor a 2, no será posible obtener puntos de intento, ya que los puntos de intento corresponden a la mitad del puntaje, por lo tanto, el juego se cerrará
			if(General.puntaje<2) {
				JOptionPane.showMessageDialog(null, "No tienes puntos para intercambiar. El juego se cerrará" + General.puntaje);
				this.setVisible(false);
			}
			else {
				//Si por el contrario, se tienen puntos, serán divididos entre dos y se obtendrá este valor como puntos de intento
				//Se invoca el método IntentosNuevos
				intentos=Intentosnuevos(intentos);
				//Se invoca el método Puntosnuevos
		...
		//Si se hace click en el botón de reto, se creará la clase Reto1
		if (e.getSource().equals(BotonReto)) {
			Reto1 Reto1 = new Reto1();
		}
	}
}
//Aquí se indica lo que ocurre si se hace click en el botón de reto
//RETO 1
//Se crea la clase Reto1
class Reto1 extends Retos implements ActionListener{
	public Reto1() {
		super();
		//Se llama al método PreguntaReto01
		PreguntaReto01();
		
	}
	//Se indica lo que sucede al hacer click en el botón verificar
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(BotonVerificar)) {
			//Si cada uno de los cuadros de texto concuerda con las variables Letra1, Letra2, Letra3, Letra4 y Letra5, se despliega un mensaje indicando que la respuesta es correcta y se obtendrá un punto de intentos
			if(Letra1.getText().contentEquals("I") && Letra2.getText().contentEquals("R") && Letra3.getText().contentEquals("O") && Letra4.getText().contentEquals("E") && Letra5.getText().contentEquals("N")) {
				...
				//Se elimina el botón de reto de la ventana para que no pueda volver a ser usado
				...
				this.setVisible(false);
				//Se elimina el botón de reto para que no pueda volver a ser usado
				GeneralAbiertos.BotonReto.setVisible(false);
			}
	}
}
...
class Desafio02 extends GeneralAbiertos implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(BotonConseguir)) {
			if(General.puntaje<2) {
				JOptionPane.showMessageDialog(null, "No tienes puntos para intercambiar. El juego se cerrará");
				this.setVisible(false);
			}
			else {
				intentos=Intentosnuevos(intentos);
